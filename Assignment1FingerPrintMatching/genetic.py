...
def match_hough(mnt1,mnt2, d_theta=0,d_x=0,d_y=0):
    ...
    for i in range(0,M):
        for j in range(0,N):
            ...
            if(f1[i]==0 and f2[j]==0 and mnt1[i][3]==mnt2[j][3]and rot_ij<15 and dij<15):
                f1[i]=1
                f2[j]=1
                count+=1
...
ans=0
for d_x in range(-20 ,20,5):
    for d_y in range(-20,20,5):
        for d_theta in range(-20,20,5):
            mnt2=transform(mnt2,d_theta,d_x,d_y)
            ans=max(ans,match_hough(mnt1,mnt2))
